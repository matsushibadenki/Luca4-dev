...
class Config:
    ...
    # AI_sandbox関連の設定
    SANDBOX_DATABASE_URL: str = os.getenv("SANDBOX_DATABASE_URL", "sqlite:///./sandbox_manager.db")
    SANDBOX_BASE_IMAGE: str = os.getenv("SANDBOX_BASE_IMAGE", "python:3.10-slim-bookworm")
    SANDBOX_RESOURCE_LIMITS: Dict[str, Any] = {
        "mem_limit": os.getenv("SANDBOX_MEM_LIMIT", "2g"), # メモリ制限
        "cpu_period": int(os.getenv("SANDBOX_CPU_PERIOD", "100000")),
        "cpu_quota": int(os.getenv("SANDBOX_CPU_QUOTA", "50000")), # 0.5 CPU
        "pids_limit": int(os.getenv("SANDBOX_PIDS_LIMIT", "50")),
    }
    SANDBOX_NETWORK_MODE: str = os.getenv("SANDBOX_NETWORK_MODE", "sandbox_network")
    SANDBOX_CONTAINER_LABELS: Dict[str, str] = {"com.example.type": "luca_sandbox"} # ラベルをLuca用に変更
    SANDBOX_TIMEOUT_SECONDS: int = int(os.getenv("SANDBOX_TIMEOUT_SECONDS", "60"))

    # ユーザーとの共有ディレクトリ設定 (ホストOS上のパスとコンテナ内のマウントポイント)
    # Lucaのルートディレクトリからの相対パスになるように調整
    SHARED_DIR_HOST_PATH: str = os.getenv("SHARED_DIR_HOST_PATH", os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), "AI_sandbox_shared_files")))
